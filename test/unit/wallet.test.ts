...
describe('Wallet tests', () => {
  ...
  it('replaces utxo', async () => {
    ...
    expect(w.selectedUtxos.items[0].balance).toBe(900)
    expect(w.getTxFund(w.lastTx)).toBe(100)
    expect(w.selectedUtxos.count).toBe(1)
    expect(w.selectedUtxos.items[0].status).toBe('hold')
    expect(w.balance).toBe(900)
    w.spendUtxos(buildResult.utxos, buildResult.tx, 0, buildResult.tx.id())
    expect(w.balance).toBe(900)
    expect(w.selectedUtxos.count).toBe(2)
    expect(w.selectedUtxos.items[0].status).toBe('spent')
    expect(w.selectedUtxos.items[1].status).toBe('available')
    expect(w.selectedUtxos.spendable().count).toBe(1)
    expect(w.selectedUtxos.spent().count).toBe(1)
    const buildResult2 = await w.makeStreamableCashTx(Long.fromNumber(100))
    expect(w.balance).toBe(800)
    expect(w.selectedUtxos.count).toBe(2)
    expect(w.selectedUtxos.items[0].status).toBe('spent')
    expect(w.selectedUtxos.items[1].status).toBe('hold')
    w.spendUtxos(buildResult2.utxos, buildResult2.tx, 0, buildResult2.tx.id())
    expect(w.balance).toBe(800)
    expect(w.selectedUtxos.count).toBe(3)
    expect(w.selectedUtxos.items[0].status).toBe('spent')
    expect(w.selectedUtxos.items[1].status).toBe('spent')
    expect(w.selectedUtxos.items[2].status).toBe('available')
  })
